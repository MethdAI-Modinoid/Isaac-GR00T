    ...
    def receive_process(self):
        ...
        try:
            while self.running:
                ...
                self.current_image = cv2.imdecode(np_img, cv2.IMREAD_COLOR)
                if self.current_image is None:
                    logger_mp.warning("[Image Client] Failed to decode image.")
                    continue

                if self.tv_enable_shm:
                    np.copyto(self.tv_img_array, np.array(self.current_image[:, :self.tv_img_shape[1]]))
                
                if self.wrist_enable_shm:
                    np.copyto(self.wrist_img_array, np.array(self.current_image[:, -self.wrist_img_shape[1]:]))
                
                if self._image_show:
                    height, width = self.current_image.shape[:2]
                    # resized_image = cv2.resize(self.current_image, (width // 2, height // 2))
